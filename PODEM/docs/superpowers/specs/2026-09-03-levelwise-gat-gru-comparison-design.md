...
新增 `SmartATPG-GAT-GRU-64`，作为第二种可训练的图策略模型，与现有
...
每个 gate 继续使用现有的 11 维输入特征。首先通过一个可训练线性层得到
初始 hidden state：
...
- GAT 投影矩阵：数学形状为 `64 x 64`，PyTorch linear weight 形状为
  `[64, 64]`。
- Attention 向量：形状为 `[128]`，作用于变换后 target 与 source state
  的拼接结果。
- GRU input size：64。
- GRU hidden size：64。

最终 gate embedding 严格为 64 维。两位 action mask 不属于 embedding，
只在构造决策描述符时追加，因此 Actor 接收的 policy state 为 66 维。
...
- 新建独立的 GAT-GRU 模块，负责 64 维编码器、Policy 和 PPO Agent 变体。
...
- 原生 C++ Actor loader 新增对 64 维 embedding 和 66 维 policy state 的
  支持，同时保留现有 11/13 维基线支持。
...
使用新的版本化模型格式和 graph configuration ID，防止基线模型与 64 维
embedding 被错误配对。

新模型与 embedding 工件必须验证：

- encoder variant 与 graph configuration；
- gate embedding 维度 64 和 policy state 维度 66；
...
单元测试必须验证输入升维层尺寸、正反向参数互相独立、attention 归一化、
同 level 同步更新、正向和反向信息流、64 维 embedding 以及 66 维决策描述符。
梯度测试必须证明梯度能够到达输入升维层、两个方向的 GAT、两个 GRU cell 和
Actor。
